...
export function RiderLocationMap({
  ...
  const centerPoint = riderPoint ?? pickupPoint ?? destinationPoint;

  if (!isValidLocationPoint(centerPoint)) {
    return null;
  }

  console.log('TRACK_MAP_SKIPPED_SAFE_MODE', {
    hasDestinationPoint: isValidLocationPoint(destinationPoint),
    hasPickupPoint: isValidLocationPoint(pickupPoint),
    hasRiderPoint: isValidLocationPoint(riderPoint),
  });
...
function isValidLocationPoint(point: LocationPoint | null) {
  return (
